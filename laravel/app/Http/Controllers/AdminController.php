...
public function banUser(Request $request, $id): JsonResponse
{
    ...
    try {
        ...
        if ($user->is_admin == 1 && $admin->is_founder != 1) {
            return response()->json([
                'success' => false,
        ...
        // ✅ Only update the 'banned' column that exists
        $user->banned = 1;
        ...
        if (!$saved) {
            throw new \Exception('Failed to save user ban status');
        }
        ...
        try {
            ...
            \Log::warning('Token deletion failed', [
                'user_id' => $user->id,
   ...
   public function unbanUser($id): JsonResponse
{
    try {
        ...
        // ✅ Only update the 'banned' column
        $user->banned = 0;
        
        // Check if banned_at exists
        $columns = \Schema::getColumnListing('users');
        if (in_array('banned_at', $columns)) {
            $user->banned_at = null;
        }
